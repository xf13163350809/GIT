...
class IPaging
{
    ...
    public function getPageBar($url='', $attrs='')
    {
        ...
        $select = "<span>到 第 <select id='page_bar_select' style='height:18px; width:40px;'><option value=''>&nbsp;</option>{$select}</select> 页 <input class='tuik_btn' type='button' onclick="."page_select_subm('{$baseUrl}');"." value='确定'></span>";
        return $tem."<span>共{$this->totalpage}页</span>{$select}</div>";
    }
}
?>
